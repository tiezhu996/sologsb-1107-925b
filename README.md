...
- `version(3)`：新增 `mouldRepairs` 修补记录表（按 `mouldId`、`repairDate` 索引），登记每张帘子历年来的修补人、日期、换丝长度与说明。
- 数据库首次创建时通过 `populate` 写入 5 张纸帘、5 个纤维料批、8 槽抄纸工序、6 个成纸样本和 4 条纸帘修补记录。
...
| `/moulds` | 纸帘台帐 | 筛选纸帘，登记新纸帘，实时推算网目密度，登记修补人、日期、换丝长度与说明，累计修补次数 |
| `/fibers` | 纤维料批台账 | 按原料和打浆度筛选、比较，展开查看关联抄纸工序 |
| `/runs` | 抄纸工序记录台 | 按日期和帘号筛选，登记工序并即时判断 ±0.2 mm 帘纹偏差，纸帘下拉仅列出在用帘 |
